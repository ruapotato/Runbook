...
CORE_SRC = core/util.c core/ship.c core/recorder.c core/world.c \
           core/proto.c core/serve.c core/box.c core/boxcheck.c core/fight.c
...
.PHONY: all check machine fight client determinism clean windows gdext run shot
...
# (handoff §15).
check: fight machine client determinism

# THE MACHINE (decision 13). The moat: a real RV64IM computer under the ship,
# with a real shell, real files, and the ship one command away.
machine: $(BIN)
	@./$(BIN) --machine

# THE BALANCE HARNESS. Two bots fly the same forty fights: one that only ever
# shoots, and one that also puts fires out and moves people out of vacuum. The
# gate asserts a band between them -- if shooting alone wins, the ship is
# decoration; if playing well cannot win, the fight is unfair. Every constant
# in ship.c was set by running this, not by argument.
fight: $(BIN)
	@./$(BIN) --fight
...
game/bin/librunbook.linux.x86_64.so: $(CORE_SRC) $(MACHINE_SRC) gdext/runbook_gdext.c
	@mkdir -p game/bin
